...
describe('AuthenticationNavigationService', () => {
  ...
  it('Given_UnauthenticatedUser_When_loginIsCalled_Then_MicrosoftRedirectStartsForAssistantCore', () => {
    // Given
    const msalService = jasmine.createSpyObj<MsalService>('MsalService', [
      'loginRedirect',
    ]);
    ...
    const applicationNavigationService =
      jasmine.createSpyObj<ApplicationNavigationService>(
        ...
        { getApplicationAbsoluteUrl: 'http://localhost:4200/app' },
      );
    ...
    expect(msalService.loginRedirect).toHaveBeenCalledWith({
      redirectStartPage: 'http://localhost:4200/app',
