...
describe('TechnicalErrorInterceptor', () => {
  ...
  it('Given_AFailedOptionalEndpoint_When_interceptIsCalled_Then_TechnicalErrorIsNotReported', async () => {
    // Given
    ...
    const interceptor = new TechnicalErrorInterceptor(technicalErrorService);
    const request = new HttpRequest('GET', '/api/models');
  ...
  it('Given_AFailedUsageSummaryCall_When_interceptIsCalled_Then_TechnicalErrorIsNotReported', async () => {
    // Given
    const error = new HttpErrorResponse({ status: 500 });
    const next = jasmine.createSpyObj<HttpHandler>('HttpHandler', ['handle']);
    next.handle.and.returnValue(throwError(() => error));
    ...
    const interceptor = new TechnicalErrorInterceptor(technicalErrorService);
    const request = new HttpRequest('GET', '/api/usage');
  ...
  it('Given_AccessDeniedResponse_When_interceptIsCalled_Then_TechnicalErrorIsNotReported', async () => {
    // Given
